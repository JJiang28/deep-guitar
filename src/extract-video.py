...
cap = cv2.VideoCapture('./dataset_raw/Intro/Video/Intro1.mp4')
...
data = [('time', 'index', 'middle', 'ring', 'pinky')]
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                if handedness.classification[0].label == 'Right':
                    ...
                    if current_time - start_time >= 1:
                        counter += 1
                        data.append((cap.get(cv2.CAP_PROP_POS_MSEC)/1000, index_distance, middle_distance, ring_distance, pinky_distance))
                        start_time = current_time
        cv2.waitKey(1)
    else:
        break

with open('./src/data/newdata.csv', 'w', newline='') as csvfile:
